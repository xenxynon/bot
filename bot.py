...
# Auth decorators (no wraps needed — pyrogram uses func name for nothing critical)
def auth(fn):
    async def w(c,msg,*a,**kw):
        if msg.from_user and _ok(msg.from_user.id): return await fn(c,msg,*a,**kw)
    return w

def superauth(fn):
    async def w(c,msg,*a,**kw):
        if msg.from_user and _su(msg.from_user.id): return await fn(c,msg,*a,**kw)
    return w
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(30,read=None)) as h:
                with open(path,"rb") as f:
                    r = await h.post(f"https://{sv}.gofile.io/contents/uploadfile",files={"file":(name,f)})
...
# TG upload
async def _tgup(client, msg, path, status):
    ...
    if size > TG_MAX_SIZE:
        ...
        return
    await client.send_document(msg.chat.id,path,caption=f"`{name}` — {fsize(size)}",
                ...
                async with h.stream("GET",url) as r:
                    ...
                    async for chunk in r.aiter_bytes(DL_CHUNK_SIZE):
                        ...
                        if now-ts[0]>=PROGRESS_INTERVAL:
                            ts[0]=now; await _edit(status,_ptext(name,done,tot,now-t0))
...
async def _sfup(status, path, project, folder):
    ...
    async with asyncssh.connect("frs.sourceforge.net",
                                username=SF_USER,password=SF_PASS,known_hosts=None) as conn:
...
async def _upload(client, msg, path, status, do_tg, do_gf, t0):
    ...
    if do_tg: await _tgup(client,msg,path,status); res.append("tg ✓")
    if do_gf:
        if do_tg: await _edit(status,f"↑ `{name}` → gofile…")
        res.append(f"gofile: {await _gofile(path,status)}")
    await _edit(status,f"done\nfile: `{name}`\nsize: {fsize(size)}\ntime: {ftime(time.time()-t0)}\n"+"\n".join(res))

async def _dl(*, client,msg,status,uid,name,dest,t0,url=None,tg_media=None,
               ev=None,then_upload=False,do_tg=True,do_gf=False):
    try:
...
def _sfses(uid):
    now=time.time()
    for k in [k for k,v in psf.items() if now-v["ts"]>SF_SESSION_TTL]: del psf[k]
    return psf.get(uid)

async def _sfexec(status,path,project,folder):
    try:
        link=await _sfup(status,path,project,folder)
        await _edit(status,f"done\nfile: `{os.path.basename(path)}`\n{project}/{folder}\nlink: {link}")
    except Exception as e: await _edit(status,f"sf failed: `{e}`")
...
async def cmd_status(_,msg):
    ...
    if uid in transfers:
        t=transfers[uid]; parts.append(f"**xfer:** `{t['name']}` {ftime(time.time()-t['start_time'])}")
    if uid in shells:
...
async def cmd_download(client,msg):
    uid=msg.from_user.id
    if uid in transfers: await msg.reply(BUSY_MSG,quote=True); return
    r=msg.reply_to_message; media=r and (r.document or r.video or r.audio or r.photo)
    if media:
        ...
        _mktask(uid,_dl(client=client,msg=msg,status=status,uid=uid,name=name,dest=dest,t0=t0,tg_media=media))
        return
    a=_args(msg,n=2)
    ...
    _mktask(uid,_dl(client=client,msg=msg,status=status,uid=uid,name=name,dest=dest,t0=t0,url=url,ev=ev))
...
async def cmd_transfer(client,msg):
    ...
    if media:
        ...
        dest=os.path.join(os.getcwd(),name); t0=time.time()
        transfers[uid]={"type":"download","name":name,"start_time":t0}
        _mktask(uid,_dl(client=client,msg=msg,status=await msg.reply(f"↓↑ `{name}`…",quote=True),
                        uid=uid,name=name,dest=dest,t0=t0,tg_media=media,
                        then_upload=True,do_tg=do_tg,do_gf=do_gf),cleanup=lambda:_rm(dest)); return
    ...
    if not target.startswith(("http://","https://")):
        ...
        transfers[uid]={"type":"upload","name":name,"start_time":t0}
        async def _lu():
            try: await _upload(client,msg,target,await msg.reply(f"↑ `{name}`…",quote=True),do_tg,do_gf,t0)
    ...
    _mktask(uid,_dl(client=client,msg=msg,status=await msg.reply(f"↓↑ `{name}`…",quote=True),
                    ...
                    then_upload=True,do_tg=do_tg,do_gf=do_gf),cleanup=lambda:_rm(dest))
...
async def cmd_gofile(_,msg):
    ...
    try: link=await _gofile(path,status); await _edit(status,f"done\nsize: {fsize(size)}\nlink: {link}")
    except Exception as e: await _edit(status,f"failed: `{e}`")
...
async def cmd_sf(_,msg):
    ...
    if not os.path.isfile(path): await msg.reply(f"not found: `{path}`",quote=True); return
    if "yaap" in flags:
        await _sfexec(await msg.reply("↑ xenxynon-roms/yaap…",quote=True),path,SF_YAAP_PRJ,SF_YAAP_DIR); return
    if len(pos)>=2:
        await _sfexec(await msg.reply(f"↑ bot-uploads/{pos[1]}…",quote=True),path,SF_PROJECT,pos[1]); return
    uid=msg.from_user.id
    ...
    await msg.reply(f"folder for `{os.path.basename(path)}`:",
        reply_markup=InlineKeyboardMarkup([r1,r2,[InlineKeyboardButton("custom…",callback_data="sf:__custom__")]]),
        quote=True)
...
async def cb_sf(_,cq):
    ...
    await _sfexec(cq.message,info["path"],SF_PROJECT,choice)

@app.on_message(filters.text & ~filters.regex(r"^/"))
@auth
async def catch_sf_custom(_,msg):
    uid=msg.from_user.id; info=_sfses(uid)
    if not info or not info.get("awaiting_custom"): return
    folder=msg.text.strip()
    if not folder: return
    psf.pop(uid)
    await _sfexec(await msg.reply(f"↑ bot-uploads/{folder}…",quote=True),info["path"],SF_PROJECT,folder)
...
async def cmd_stdin(_,msg):
    ...
    proc=shells[uid].get("proc")
    if proc and proc.stdin:
        try: proc.stdin.write((text+"\n").encode()); await proc.stdin.drain(); await msg.reply("✓",quote=True)
        except Exception as e: await msg.reply(f"error: `{e}`",quote=True)
    else: await msg.reply("n/a",quote=True)

for _c,_s in {"ps":"ps aux --sort=-%cpu | head -30","top":"top -bn1 | head -40",
              "free":"free -h","uptime":"uptime","whoami":"id","netstat":"ss -tulnp"}.items():
    def _mk(sc):
        @auth
        async def _h(_,msg): asyncio.create_task(_runsh(msg,sc))
        return _h
    app.on_message(filters.command(_c))(_mk(_s))
    ...
    try:
        ...
        for e in entries:
            try:
                ...
                rows.append(f"{'d' if e.is_dir() else 'l' if e.is_symlink() else '-'} {fsize(st.st_size):>10}  {e.name}{'/' if e.is_dir() else ''}")
            except: rows.append(f"? {'?':>10}  {e.name}")
        await msg.reply(f"`{os.path.abspath(path)}` ({len(entries)})\n```\n"+"\n".join(rows)+"\n```",quote=True)
    except Exception as e: await msg.reply(f"error: `{e}`",quote=True)
...
async def cmd_rm(_,msg):
    ...
    try:
        t=a[0]
        if os.path.isdir(t) and not os.path.islink(t): shutil.rmtree(t); await msg.reply(f"✓ `{t}`",quote=True)
        else: os.remove(t); await msg.reply(f"✓ `{t}`",quote=True)
    except Exception as e: await msg.reply(f"error: `{e}`",quote=True)
...
def _2path(name,fn):
    ...
    async def _h(_,msg):
        ...
        try: fn(a[0],a[1]); await msg.reply(f"✓ `{a[0]}` → `{a[1]}`",quote=True)
        except Exception as e: await msg.reply(f"error: `{e}`",quote=True)

_2path("mv",shutil.move)
_2path("cp",shutil.copy2)
